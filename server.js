...
app.delete('/class/:classId/file/:fileId', async (req, res) => {
    try {
        ...
        // Just pull from MongoDB. Cloudinary files can stay in the cloud 
        // or be deleted manually from the Cloudinary dashboard to save time.
        classroom.files.pull(req.params.fileId);
        await classroom.save();
        
        res.json({ success: true, message: "File record removed" });
    } catch (err) { res.status(500).json({ error: "Error removing file record" }); }
});
